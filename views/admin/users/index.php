...
<div class="card shadow-sm border-0 rounded overflow-hidden">
        ...
        <div class="table-responsive">
            <table class="table table-hover align-middle mb-0 text-nowrap">
                <thead class="bg-light">
                    <tr>
                        <th class="ps-4 py-3 text-muted text-uppercase fw-semibold" style="font-size: 0.75rem; letter-spacing: 0.5px;">STT</th>
                        <th class="py-3 text-muted text-uppercase fw-semibold" style="font-size: 0.75rem; letter-spacing: 0.5px;">Họ và tên</th>
                        <th class="py-3 text-muted text-uppercase fw-semibold" style="font-size: 0.75rem; letter-spacing: 0.5px;">Ngày tạo</th>
                        <th class="py-3 text-muted text-uppercase fw-semibold" style="font-size: 0.75rem; letter-spacing: 0.5px;">Giờ tạo</th>
                        <th class="py-3 text-muted text-uppercase fw-semibold" style="font-size: 0.75rem; letter-spacing: 0.5px;">Trạng thái</th>
                        <th class="py-3 text-muted text-uppercase fw-semibold text-end pe-4" style="font-size: 0.75rem; letter-spacing: 0.5px;">Thao tác</th>
                    </tr>
                </thead>
                <tbody class="border-top-0">
                    <?php if (empty($users)): ?>
                        <tr><td colspan="6" class="text-center py-5 text-muted"><i class="fa fa-users fs-1 mb-3"></i><br>Chưa có dữ liệu thành viên.</td></tr>
                    <?php else: ?>
                        ...
                        <tr>
                            ...
                            <td>
                                <?php if ($user['status'] === 'active'): ?>
                                    <span class="badge bg-success bg-opacity-10 text-success border border-success px-3 py-2 rounded-1 fw-medium">Active</span>
                                <?php else: ?>
                                    <span class="badge bg-danger bg-opacity-10 text-danger border border-danger px-3 py-2 rounded-1 fw-medium">Locked</span>
                                <?php endif; ?>
                            </td>
    ...
    <div class="card-footer bg-white border-top border-light p-3 d-flex flex-column flex-sm-row justify-content-between align-items-center gap-2">
        <span class="text-muted small fw-medium">Hiển thị tất cả <strong class="text-dark"><?= count($users ?? []) ?></strong> thành viên.</span>
